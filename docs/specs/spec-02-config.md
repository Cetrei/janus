...
Resuelve la ubicación exacta de los folders de agente que `agents/03` y `agents/08` dejaron diferida.
...
6. `paths`: `state_dir` (default por plataforma con `janus_platform.default_state_dir()`: `~/.local/state/janus` en Linux y `%LOCALAPPDATA%\Janus\state` en Windows), `db_file` (default `janus.db`), `log_dir`. Se expanden `~` y variables.
...
12. `selection`: política por patrón de capacidad (glob) con `policy` (`priority`, `pinned`) y `spokes` ordenados, más `fallback` (cadenas declarativas de N pasos, ver más abajo), `role_hints` (tabla `rol -> lista de tipos de spoke preferidos`, por ejemplo `implementer = ["execution"]`), `max_route_depth` (default 4), `heartbeat_ttl_s` (default 30) y `specialties` (tabla `categoría -> tipo de agente`). Ver spec 09. `fallback` es un arreglo de tablas `[[selection.fallback]]`: cada entrada lleva `capability` (glob) o `agent` (nombre) y `steps`, una lista ordenada de pasos heterogéneos, cada uno `{ spoke = "<spoke_id>" }`, `{ spoke = "pool:<pool_id>" }`, `{ provider = "<clave de providers>" }` o `{ wait = "<duración>" }`. Ejemplo: `steps = [{ spoke = "gemini-api" }, { spoke = "pool:claude-desktop" }, { provider = "openrouter" }, { wait = "5m" }]`. Un `wait` detiene la cadena esa duración y, si nada cambió, la agota (`FallbackChainExhausted`, spec 09). Un `spoke` inexistente al cargar genera una advertencia y no un error, porque los spokes dinámicos pueden registrarse después. El alias `pool:<pool_id>` se expande a los miembros del pool en el orden de sus perfiles (spec 17). Las cadenas de proveedor de modelo de un agente siguen en `fallback_providers` de su `agent.toml` con la misma estructura de pasos `provider`.
...
19d. `gui_automation`: `detector` (`accessibility`, `vision` o `auto`, default `auto`; spec 15), `approval.<acción>` con los cuatro valores de aprobación para `type_text`, `press_key`, `click`, `activate`, `open_app`, `close_window` y `move_window` (spec 12, requisito 22bis; los defaults están en la spec 15), `allowed_apps` (ids de aplicación que `GuiAgentAdapter` puede operar; vacía significa ninguna) y `agent.max_steps` (default 30), `agent.max_duration_s` (default 300), `agent.blocked_chords` (default `["ctrl+alt+delete", "alt+f4", "ctrl+shift+q", "super+l"]`) y `agent.allow_password_fields` (default `false`).
19e. `biometrics` (spec 18): `enabled` (default `false`), `face` y `voice` con `provider`, `thresholds` y, para cara, `liveness` (`required` por defecto); `key` (`SecretRef`) o `key_source`; `sensors` (lista de `{ id, kind, device }`); `max_failed_attempts`, `lockout_window_s`, `lockout_s`, `allow_remote`, `remote_ack`, `model_idle_unload_s` y `latency_warn_ms`. Por canal, `identity = { signals, mode, control_mode, on_low_confidence, sensor }`: `signals` es un subconjunto de `platform_id`, `challenge`, `voice` y `face`; `mode` es `any` (basta una señal activa) o `all` (todas); `control_mode` rige las órdenes de control y las aprobaciones (default `all` si hay más de una señal activa y `mode` si hay una); `on_low_confidence` es `challenge` (default) o `deny`. Validación cruzada: `voice` o `face` en `signals` exige `biometrics.enabled = true`; `challenge` en `signals` o `on_low_confidence = "challenge"` exige `owner_challenge_file`; `sensor` debe existir en `biometrics.sensors`; `allow_remote = true` sin `remote_ack = true` es error.
19f. `instance_pools` (spec 17): arreglo de tablas con `pool_id`, `adapter`, `settings`, `max_alive`, `idle_close_s`, `autostart` y `profiles` (cada perfil con `profile_id`, `label`, `user_data_dir`, `launch_args`, `preferred_roles`, `adopt`, `allow_close_adopted` y `settings`).
19g. `usage` (spec 17): `warn_fraction` (default 0.85), `block_fraction` (default 0.98) y `probe_interval_s` (default 900), con override por spoke en `spokes[].usage`.
19h. `retention` (spec 03, requisito 26): `closed_sessions_days` y `terminal_tasks_days`, ambos con default `0` (sin purga automática).
19i. `judgment` (spec 11, requisito 20bis): `timeout_s` (default 15), `failure_triage_mode` (`janus` por defecto o `chain_only`), `failure_triage_skip_kinds` (default vacío; por ejemplo `["quota"]` para ahorrar tokens), `failure_triage_default` (`continue_chain`) y `dependency_triage_default` (`ask_user`), que son el veredicto si el tiempo vence.
...
* **Portability**: Python 3.11 o superior. Dependencias permitidas: `pydantic>=2`, stdlib y `janus_platform`. Prohibido importar `libs/adapters`, `libs/persistence`, `libs/capabilities`, `libs/proto-py` y `apps/*`. Los valores como `on_dependency_failure` se manejan como cadenas y los mapea el consumidor.
...
* **Reason**: `config/` ya es el territorio editable por el usuario y por IA (`config/janus.toml`); reúne lo declarado por el usuario y mantiene `docs/` como documentación.
...
Entity JanusSettings { core, paths, redis, harnesses, spokes[], providers{}, selection, failure,
                       agents, approval, memory, voice, observability, identity, gui_automation, biometrics, instance_pools[], usage, retention, judgment }
...
- [x] `filesystem_roots`: la validación (cada raíz existe y ninguna incluye `config/agents` de otros agentes salvo que el agente sea `Janus`) es un requisito de la carga, no una pregunta abierta. Usa `janus_platform.path_is_within`.
...
- [x] Puertos por defecto: `core.grpc_port = 47080`, `core.mcp_port = 47081` y `harnesses.channel-gateway.port = 47090`. El rango 47080 a 47099 queda reservado para Janus y evita 8000 y 8080, que otros servicios locales usan con frecuencia.
