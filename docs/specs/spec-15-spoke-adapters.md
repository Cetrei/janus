...
> **Orden de implementación**: 15 de 20. Depende de: spec 04 (contratos), spec 10 (motor), spec 11 (núcleo), spec 12 (GUI), spec 14 (channel-gateway), spec 16 (`janus_platform`) y spec 17 (pools de instancias, absorción de Relay). La elección de candidato y la detección de cuota vía decision model (requisitos 25, 28 y 32bis-vision) dependen opcionalmente de la spec 19; los tres requisitos funcionan igual sin ella con su criterio previo (orden de lectura, `quota_patterns`, o el modelo de visión decidiendo la coordenada directamente).
...
25. `GuiChatAdapterBase` (`GuiDrivenMixin` más `ReasoningAdapter`, `max_concurrency == 1`) implementa el flujo genérico de las aplicaciones de chat: adquirir `screen_lock`, localizar o abrir la ventana, enfocar y `verify_focus`, mapear elementos, `expect` de los anclajes de entrada, envío y salida, opcionalmente iniciar una conversación nueva, escribir el texto de la solicitud, activar el envío, `wait_until_stable` y leer la respuesta. Devuelve un terminal `SUCCEEDED` con el texto (más `PROGRESS` "generando" opcionales). Usa el protocolo `GuiSurface` de `janus_gui.surface` (spec 12) y toda llamada va en `asyncio.to_thread`. **Elección de anclaje asistida por Laya (spec 19, requisito 18, integrado 2026-09-25; decisión de proveedor confirmada 2026-09-25):** los anclajes de entrada, envío y salida (`expectations`, requisito 26) suelen resolverse por rol y nombre fijos en la config; cuando el layout varía entre versiones de la app y hay más de un candidato que matchea el mismo anclaje esperado en el `ElementMap`, la base pide al decision model una `ChoiceQuestion` sobre los candidatos ya enumerados (nunca sobre elementos fuera del mapa) antes de llamar a `activate` (spec 12, requisito 14). **Laya (`laya_local`) es el modelo usado para toda decisión de interacción con interfaz de esta spec** (elección de anclaje, requisito 25; detección de cuota, requisito 28; elección de candidato en `VisionAgentAdapter`, requisito 32bis-vision), no una referencia genérica a "el decision model configurado": es la decisión explícita del usuario que motivó la creación de la spec 19, y el caso de uso de elección de elemento en GUI fue el ejemplo original con el que se justificó integrar Laya (ver spec 19, motivación). Un adaptador puede sobreescribir `decision_models` a otro backend por configuración (el puerto sigue siendo intercambiable, spec 19 requisito 12), pero el valor de fábrica de esta spec para GUI es Laya, no un placeholder genérico. Umbral de confianza: `decision.triage_confidence_threshold`-equivalente por consumidor, definido en la spec 19; por debajo del umbral, se aplica el criterio existente (primer candidato por orden de lectura) sin bloquear el flujo. Opcional: sin Laya (u otro decision model) configurado, el flujo funciona igual con ese criterio de desempate.
...
28. Detección de cuota: por defecto (`gui_automation.quota_detection = pattern_match`), si el texto de salida o la interfaz coincide con `quota_patterns`, lanza `QuotaExhaustedError(retry_after)`, con `retry_after` calculado si el mensaje de la aplicación trae la ventana de espera y una estimación configurable si no. **Detección asistida por Laya (spec 19, requisito 20, integrado 2026-09-25; decisión de proveedor confirmada 2026-09-25):** con `gui_automation.quota_detection = decision_model`, en vez de (o además de, según configuración) comparar contra `quota_patterns`, la base le hace a Laya una `NoulQuestion` ("¿este texto de salida indica que se agotó la cuota?") sobre el texto leído; si la respuesta es verdadera y su `confidence` supera el umbral configurado, se trata igual que un match de patrón. Laya es el modelo usado aquí por la misma decisión del requisito 25, no una referencia genérica; el nombre de la opción de config (`decision_model`, genérico) refleja que el puerto sigue siendo intercambiable (spec 19), pero el valor de fábrica que resuelve ese nombre es Laya. Más robusto que una lista fija si el texto de la aplicación cambia entre versiones, pero no reemplaza `quota_patterns` como mecanismo: `pattern_match` sigue siendo el default y el único requerido para que el adaptador funcione sin Laya configurado. La base publica salud `UNAVAILABLE` y el Registro elige otro spoke (`architecture/08` sección 5).
...
32bis-vision. `VisionAgentAdapter` (nombre de paquete `janus_adapters.spokes.vision_agent`), capacidad `vision.ui.locate` y `vision.ui.step` (ids reservados en spec 01, requisito 14): recibe un objetivo en lenguaje natural y opcionalmente el historial de pasos previos; captura la ventana enfocada vía `janus_gui.probe`/`CaptureBackend` (spec 12), la envía como `Payload.attachments` (spec 01, requisito recién agregado) a un modelo de visión liviano configurable (proveedor local por defecto, proveedor de nube opt-in explícito, mismo mecanismo `allow_remote`/`remote_ack` que la spec 18) y devuelve la acción propuesta (clic en coordenadas, texto a escribir, tecla a pulsar) para que la base de `GuiChatAdapterBase` la ejecute vía `activate`/`type_text`/`press_key`. Nunca ejecuta la acción por sí mismo: la decisión de mutar pantalla sigue pasando por `request_approval` (spec 04, requisito 23; spec 11, requisito 32bis) igual que cualquier otra acción de mutación de `janus_gui` (spec 12, requisito 22bis). Config (`VisionAgentSettings`): `provider` (referencia a un modelo de visión, mismo esquema de proveedor intercambiable que biometría y voz; este es un modelo de visión, distinto y sin relación de intercambiabilidad con Laya), `max_steps` (default 30, mismo límite que `gui_automation.agent.max_steps`), `confirm_before_action` (default según la política de aprobación del tipo de acción). Detalle técnico completo (modelos concretos, umbrales, formato del prompt de localización) diferido: no se fija de memoria y se verifica al implementar, siguiendo el mismo criterio que la spec 18 con los modelos de biometría. **División de trabajo con Laya (spec 19, requisito 18, integrado 2026-09-25; decisión de proveedor confirmada 2026-09-25):** el modelo de visión de este adaptador (`provider`, arriba) cumple el rol de "fijar el objetivo o interpretar la pantalla" (identificar qué hay que hacer, y en el caso de un espacio de candidatos no enumerable de antemano, proponer dónde); cuando el paso siguiente sí es elegir entre candidatos ya enumerados (por ejemplo, varios elementos del `ElementMap` de la spec 12 que matchean el mismo objetivo), esa elección puntual se delega a Laya (`ChoiceQuestion` sobre esos candidatos) en vez de pedirle al modelo de visión que razone la coordenada exacta en cada paso, siguiendo el mismo patrón que el requisito 25 de esta spec. Son dos modelos distintos con roles distintos: `provider` (visión, ve la pantalla) y Laya (decisión, elige entre opciones ya enumeradas); no se sustituyen entre sí. Esto no cambia el contrato de `vision.ui.step`: el modelo de visión sigue siendo el que decide la acción final cuando no hay candidatos enumerables; Laya solo acelera el caso en que sí los hay.
...
Config GuiChatSettings    { mapping_strategy, window_query, launch_command?, conversation_mode,
                            ...
                            quota_detection?: pattern_match|decision_model }   // default pattern_match; decision_model resuelve a Laya por defecto (spec 19 req 20)
...
- [x] Integración del decision model (spec 19) en los adaptadores GUI: resuelto (2026-09-25). `GuiChatAdapterBase` (requisito 25) puede usar una `ChoiceQuestion` para desambiguar anclajes candidatos antes de `activate`; detección de cuota (requisito 28) gana el modo `decision_model` vía `NoulQuestion`, configurable con `gui_automation.quota_detection`; `VisionAgentAdapter` (requisito 32bis-vision) delega al decision model la elección entre candidatos ya enumerados, dejando al modelo de visión el rol de fijar el objetivo. Los tres usos son opcionales y no cambian el comportamiento por defecto sin decision model configurado.
